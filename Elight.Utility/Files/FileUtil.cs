...
namespace Elight.Utility.Files
{
    ...
    public class FileUtil
    {
        ...
        public static List<FileExt> ReadLines(string filePath)
        {
            string[] strLines = File.ReadAllLines(filePath);
            List<FileExt> lineList = new List<FileExt>();
            foreach (string str in strLines)
            {
                ...
                {
                    Name = splitArray[0],
                    ColName = splitArray[1],
                    Width = Convert.ToInt32(splitArray[2])
                };
                lineList.Add(ext);
            }
            return lineList;
        ...
        {
            strBase64 = "eyJlcnJjb2RlIjo0MTAQingDaiwiZXJybXNnIjoiYWNjZXNzX3Rva2VuIG1pc3NpbmcgaGludDogW1JScGREQTA4ODl2cjI5IV0ifQ==";
            ext = "amr";
            byte[] contents = Convert.FromBase64String(strBase64);
            string fileName = DateTime.Today.ToString("yyyyMMdd") + Guid.NewGuid().ToString().Replace("-", "") + "." + ext;
